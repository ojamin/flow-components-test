...
import type { ComponentCategory, ComponentGroupId } from "./taxonomy";
import type { ComponentThemeContext } from "./theme";
import type { ComponentThemeContract } from "./theme-contract";
...
export interface EventOutputInitialContext<
...
export interface EventOutputBinding<
  TConfig extends Record<string, unknown> = Record<string, unknown>,
> {
  eventId: string;
  outputId: string;
...
export interface RuntimeExecutionContext<
  TConfig extends Record<string, unknown> = Record<string, unknown>,
  TInputs extends Record<string, unknown> = Record<string, unknown>,
  TFixtureData = unknown,
> extends TransformContext<TConfig, TInputs> {
  configValid: boolean;
...
> {
  evaluate: (
    context: RuntimeExecutionContext<TConfig, TInputs, TFixtureData>,
  ) => RuntimeExecutionResult<TOutputs> | Promise<RuntimeExecutionResult<TOutputs>>;
}

export type ComponentConfig<TSchema extends ZodTypeAny> = z.output<TSchema> &
  Record<string, unknown>;
export type ComponentConfigInput<TSchema extends ZodTypeAny> = z.input<TSchema> &
  Record<string, unknown>;
export type PortOutputMap<TOutputs extends readonly OutputPortDefinition[]> = Partial<
  Record<TOutputs[number]["id"], unknown>
>;
...
export type StateApplicability = true | { notApplicable: string };
...
export type DataBoundarySourceKind =
  | "root"
  | "http"
  | "file"
  | "dataset"
...
export type DataBoundaryMetric =
  | "bytes"
  | "rows"
  | "lines"
  | "objects"
...
export type DataBoundaryExportBehavior =
  | "live"
  | "embedded"
  | "fallback"
  | "not-applicable"
...
export type ComponentDefinitionInput<TSchema extends ZodTypeAny = ZodTypeAny> = Omit<
  ComponentDefinition<TSchema>,
  "configDefaults" | "slots"
> & {
  slots?: SlotDefinition[];
  configDefaults: ComponentConfigInput<TSchema>;
};

export interface StaticComponentRenderProps<
  TConfig extends Record<string, unknown> = Record<string, unknown>,
  TFixtureData = unknown,
> {
 ...
 */
export type StaticComponentMapLibreDiagnosticsStatus =
  | "empty"
  | "loading"
  | "ready"
  | "no-layers"
...
export type StaticComponentMapLibreDiagnosticsWebGLSupport =
  | "webgl2"
  | "webgl"
  | "experimental-webgl"
  | "unsupported"
 ...
 *   was not provided.
 */
export type StaticComponentVirtualizationAuditStatus =
  | "not-needed"
  | "exact"
  | "bounded-authoring"
  | "bounded-paginated"
 ...
 */
export type StaticComponentVirtualizationAuditRenderMode =
  | "authoring"
  | "exact"
  | "export"
  | "unknown";
...
export interface StaticComponentTransformContext<
  TConfig extends Record<string, unknown> = Record<string, unknown>,
  TInputs extends Record<string, unknown> = Record<string, unknown>,
  TFixtureData = unknown,
> {
...
export interface StaticComponentTransformModule<
  TConfig extends Record<string, unknown> = Record<string, unknown>,
  TInputs extends Record<string, unknown> = Record<string, unknown>,
  TFixtureData = unknown,
  TOutputs extends Record<string, unknown> = Record<string, unknown>,
> {
  ...
  transform: (
    context: StaticComponentTransformContext<TConfig, TInputs, TFixtureData>,
  ) => TOutputs | Promise<TOutputs>;
}
